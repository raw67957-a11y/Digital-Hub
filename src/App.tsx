...
export default function App() {
    ...
    "prod-2": [
      {
        ...
        rating: 5,
        comment:
          "Peaceful ambient videos, perfect to edit and post directly. Thanks for the quick support!",
        date: "2 days ago",
      },
    ],
    "prod-3": [
      {
        ...
        rating: 5,
        comment:
          "Are public link working? Yes, downloaded in 10 secs. Unreal price ₹15!",
        date: "Just now",
      },
    ],
  });
  ...
  const [settings, setSettings] = useState(() => {
    ...
    return {
      ...
      phone: "+91 96235 08876",
      video1:
        "https://player.vimeo.com/external/371433846.sd.mp4?s=236da2f3c022f73bcf7407d60f04e22596ab5933&profile_id=165&oauth2_token_id=57447761",
      video2:
        "https://player.vimeo.com/external/434045526.sd.mp4?s=c27db11cf4ca9aa14704e6c310fb4e067fd4b39b&profile_id=165&oauth2_token_id=57447761",
      video3:
        "https://player.vimeo.com/external/403842104.sd.mp4?s=d7fb47da2f1464b5849dfb0c95a2879f91a5ad56&profile_id=165&oauth2_token_id=57447761",
    };
  });
  ...
  const handleDeleteUserAccess = (id: string) => {
    setUserAccesses((prev) =>
      prev.filter((ua) => ua.id !== id && ua.userId !== id),
    );
  };
      ...
      try {
        ...
        if (orderId && productId) {
          ...
          if (matched) {
            ...
            // Clean browser URL directly early
            const newUrl = window.location.pathname + window.location.hash;
            window.history.replaceState({}, document.title, newUrl);

            // Contact Backend to verify order natively in Cashfree
            const reqBody = { 
              order_id: orderId, 
              isDemo: orderId.startsWith("demo_order_") 
            };
            ...
            try {
              ...
              if (verifyData.success && verifyData.verified) {
                ...
                // Payment not completed or failed verification
  ...
  return (
    <div className="min-h-screen bg-[#0a0708] text-white flex flex-col justify-between selection:bg-[#fbbf24] selection:text-black">
      ...
      <main className="max-w-7xl w-full mx-auto px-4 sm:px-6 md:px-8 py-8 sm:py-12 flex-grow space-y-12">
        ...
        <div className="text-center space-y-3 max-w-3xl mx-auto py-2">
          ...
          <div className="inline-flex items-center gap-1.5 bg-[#140f10] border border-[#231a1c] px-3.5 py-1 rounded-full text-xs text-gray-400 font-semibold mb-2">
            ...
            <span>Limited Offer: Instant Drive Access</span>
          </div>
          ...
          {filteredProducts.length === 0 && (
            <div className="col-span-full py-12 text-center space-y-3 bg-[#140f10] rounded-3xl border border-[#231a1c]">
              <div className="text-gray-600 text-sm">
                No digital products match your filter search.
              </div>
              <button
                onClick={() => {
                  setSearchQuery("");
      ...
      {/* Payment Failed / Not Successful */}
      {paymentFailedData && (
        <PaymentFailedModal
          product={paymentFailedData.product}
          customerName={paymentFailedData.name}
          customerEmail={paymentFailedData.email}
          customerPhone={paymentFailedData.phone}
          onRetry={() => {
            setSelectedProduct(paymentFailedData.product);
            setPaymentFailedData(null);
          }}
          onClose={() => setPaymentFailedData(null)}
        />
      )}

      {/* Payment Verification Overlay Loader */}
      {isVerifyingPayment && (
        <div className="fixed inset-0 z-[200] flex flex-col items-center justify-center bg-[#0a0708]/90 backdrop-blur-md">
          <div className="w-12 h-12 border-4 border-amber-500/20 border-t-amber-500 rounded-full animate-spin mb-4" />
          <h2 className="text-xl font-bold text-white tracking-tight">Verifying Payment...</h2>
          <p className="text-sm text-gray-400 mt-2">Please wait establishing secure connection with bank.</p>
        </div>
      )}
      ...
      <footer
        id="about-section"
        className="bg-[#0a0708] border-t border-[#231a1c] pt-12 pb-8 px-6 sm:px-12 md:px-16 text-left selection:bg-[#fbbf24] selection:text-black"
      >
        <div className="max-w-4xl space-y-8">
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm text-gray-400 font-sans leading-relaxed max-w-xl">
              Premium digital product. Lifetime access. Instant delivery on your
              email.
            </p>
          </div>
